...
class SendCodeRequest(BaseModel):
    email: EmailStr
    code: str
    subject: Optional[str] = "Los Barryachis - Ton code de vérification"
...
def generate(request: EmailRequest, api_key: str = Depends(verify_api_key)):
    try:
        ...
        send_code_email(request.email, "Los Barryachis - Ton code de vérification", code)
...
def send_custom_code(request: SendCodeRequest, api_key: str = Depends(verify_api_key)):
    try:
        ...
        email_id = queue_email(request.email, "Los Barryachis - Ton code de vérification", html_content)
...
if __name__ == "__main__":
    import uvicorn
    print(os.path.dirname(os.path.abspath(__file__))+"\\verification.db")
